...
import { Search, BookOpen, NotebookText, BarChart3, X } from "lucide-react";
  ...
  const results = useMemo(() => {
    ...
    const documents = data.documents.filter((d) => {
      ...
      return (
        ...
        (text.textContent || "").toLowerCase().includes(q)
      );
    });
      ...
      <div className="modal-enter w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-slate-800">
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          ...
          {results.documents.map((d) => (
            <button
              key={d.id}
              onClick={() => goTo(d.kind === "course" ? `/cours/${d.id}` : `/fiches/${d.id}`)}
            ...
            >
              {d.kind === "course" ? (
                ...
                <NotebookText className="h-4 w-4 shrink-0 text-[var(--accent)]" />
              )}
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-slate-400">
                  {d.kind === "course" ? "Cours" : "Fiche"} · {subjectName(d.subjectId)}
                </span>
              </span>
            </button>
          ))}
