...
export function Dashboard() {
  ...
  const nextCourses = useMemo(() => {
    ...
    if (upcomingToday.length > 0) return upcomingToday.slice(0, 4).map((s) => ({ ...s, dayLabel: "Aujourd'hui" }));
    ...
    for (let offset = 1; offset <= 7; offset++) {
      ...
      if (daySlots.length > 0) return daySlots.slice(0, 4).map((s) => ({ ...s, dayLabel: DAYS[dayIdx] }));
    }
    return [];
  }, [schedule, settings.currentWeek]);

  const latestGrades = [...grades].sort((a, b) => b.createdAt - a.createdAt).slice(0, 5);
  ...
  const stats = [
    ...
    { label: "Cours & fiches", value: documents.length, icon: BookOpen },
  ];
              ...
              {nextCourses.map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center gap-3 rounded-xl border-l-4 bg-slate-50 px-3 py-2.5 dark:bg-slate-900/40" style={{ borderColor: s.color || subject?.color || "#6366f1" }}>
                    <div className="w-16 shrink-0 text-xs font-semibold text-slate-500">{s.dayLabel}<br />{s.start}</div>
